...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  /**
   * Safety net for the Civ 6 auto-start: the mod and PlayNowSave should only exist while a
   * turn is being played. If anything is still in place while no Civ 6 game is waiting on us
   * (client killed mid-turn, game crashed, revert deferred because the game was still running),
   * revert it so a plain launch of Civ 6 is stock. No-op when nothing is left.
   */
  private async revertStaleCiv6Autostart(turnsToNotify: Game[]): Promise<void> {
  ...
  async loadGames(): Promise<void> {
    ...
    void this.revertStaleCiv6Autostart(turnsToNotify);
